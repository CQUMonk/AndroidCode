...
import cqu.cqumonk.androidcode.ui.Game2048;
...
public class GameView extends LinearLayout{
				...
				for (int next = x-1; next>=0; next--) {
					...
					if (cardMatrix[next][y].getNum()>0) {
						...
						if (cardMatrix[x][y].getNum()<=0) {
							Game2048.getGameActivity().getAnimLayer()
							...
							Game2048.getGameActivity().addScore(cardMatrix[x][y].getNum());
							//重新判断一次
							merge = true;
						}
						break;
	...
	private void startGame() {
		Game2048.getGameActivity().startGame();
	}
	...
	public  void addRandomNum(){
		...
		if (emptyPoints.size()>0) {
			...
			Game2048.getGameActivity().getAnimLayer().createScaleTo1(cardMatrix[p.x][p.y]);
		}
	}
	private void checkComplete() {
		...
		if (complete) {
			Game2048.getGameActivity().saveBestScore();
